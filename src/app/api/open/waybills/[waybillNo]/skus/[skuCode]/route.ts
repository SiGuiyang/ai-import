...
/**
 * GET /api/open/waybills/[waybillNo]/skus/[skuCode]
 * 根据运单号和 SKU 编码查询运单信息（对外开放接口）
 *
 * 请求头鉴权：
 *   X-App-Id: <appId>
 ...
 *   skuCode   - SKU 编码（对应 orders 表的 sku_code）
 */
export async function GET(
  req: NextRequest,
  { params }: { params: Promise<{ waybillNo: string; skuCode: string }> }
) {
  // 1. 鉴权
  ...
  // 2. 获取参数
  const { waybillNo, skuCode } = await params;

  if (!waybillNo || !skuCode) {
    return NextResponse.json(
      { code: 400, message: '运单号和 SKU 编码均为必填项' },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    if (rows.length === 0) {
      return NextResponse.json(
        { code: 404, message: `运单号 ${waybillNo} 下未找到 SKU ${skuCode}` },
        { status: 404 }
      );
    }
    ...
    const waybill: Record<string, unknown> = {
      waybillNo,
      skuCode,
